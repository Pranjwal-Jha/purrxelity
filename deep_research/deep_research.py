...
def generate_message_plan(state: ReportState):
    prompt = ChatPromptTemplate.from_messages(
        [
            ...
            (
                "human",
                "Generate a report plan for the topic: {input}",
            ),  # change max section
        ]
    )
    ...
    class Sections(BaseModel):
        sections: List[SectionOutput] = Field(
            description="List of sections for the report atmost 2"
        )  # change max section
    ...
    return {
        ...
        "messages": HumanMessage(content="Sections Generated !"),
    }
...
def research_agent(state: ReportState):  # removed async
    ...
    prompt = [
        SystemMessage(
            content="You are a research agent. Your task is to research a given topic and provide a detailed summary. You have web search tool in case needed"
        ),
        HumanMessage(
            content=f"Research the following topic for the section '{section_to_research.name}', description '{section_to_research.description}'"
        ),
    ]
...
def process_search_results(state: ReportState):
    ...
    prompt = [
        SystemMessage(
            content="You are a research summarization agent. Based on the search results provided in the conversation, create a comprehensive and well-structured summary for the report section."
        ),
        HumanMessage(
            content=f"Based on all the search results above, provide a detailed, comprehensive summary for the section '{section_to_research.name}' with description '{section_to_research.description}'. Make it informative and well-organized."
        ),
    ]
...
def research_section_entry(state: ReportState):
    ...
    if len(state["completed_sections"]) < len(state["sections"]):
        return {
            "messages": [
                HumanMessage(content="Starting research for the next section.")
            ]
        }
    else:
...
def should_continue_research(
    state: ReportState,
) -> Literal["research_agent", "compile_final_report"]:
...
def compile_final_report(state: ReportState):
    ...
    return {"final_report": all_section}

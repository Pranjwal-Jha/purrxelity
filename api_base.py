...
async def deep_research(user_input: BasicChat,user_id: int, db: AsyncSession = Depends(get_db)):
    db_user = await crud.get_user_by_id(db, user_id=user_id)
    if db_user is None:
        raise HTTPException(status_code=404, detail="User not found")
...
async def login_for_access(
    user_cred: schemas.LoginRequest, db: AsyncSession = Depends(get_db)
):
    user = await crud.autheticate_user(
        db, email=user_cred.email, password=user_cred.password
    )
    if not user:
        raise HTTPException(
...
async def create_chat_history(
    ...
    created_chat_history = await crud.create_chat_history(
        db, chat_data, user_id, thread_id
    )
...
async def delete_user_chat(
    user_id: int, chat_thread_id: str, db: AsyncSession = Depends(get_db)
):
    db_user = await crud.get_user_by_id(db, user_id)
    if db_user is None:
        raise HTTPException(status_code=404, detail="User not found")
...
async def add_message_to_chat(
    ...
    updated_chat = await crud.append_message_to_chat(
        user_id, thread_id, new_messages, db
    )
    if not updated_chat:
